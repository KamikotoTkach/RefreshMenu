...
public class DynamicView extends View implements Refreshable {
  ...
  public Optional<Ingredient> getDynamic(int slot) {
    ...
    boolean isDynamic = dynamicIngredients.containsKey(countedChar);
    if (!isDynamic) return Optional.empty();
  ...
  public Optional<Ingredient> getIngredient(char character, int slot) {
    ...
    boolean isDynamic = dynamicIngredients.containsKey(character);
